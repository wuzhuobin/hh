...
std::string MyTask::jabberwocky(std::string s)
{
  ...
  // TODO
	std::string name_ = s;
	for (size_t i = 0; i < name_.size() - 1; i += 2) {
		char temp = name_[i];
		name_[i] = name_[i + 1];
		name_[i + 1] = temp;
	}
...
void MyTask::finish_the_work(void)
{
  ...
  // TODO
	std::fstream fout;
	fout.open("letter", std::fstream::out);
	for (std::string str : this->data_) {
		for (char c : str) {
			for (uint64_t i = 0; i < this->sk_; ++i) {
				c *= c;
			}
			c %= this->sn_;
			fout << c;
	...
	fout.close();
	fout.open("key.pub", std::fstream::out);
	fout << std::to_string(this->pk_);
	fout << "\n";
	fout << std::to_string(this->sn_);
	fout.close();
	fout.open("key.prv", std::fstream::out);
	fout << std::to_string(this->sk_);
	fout << "\n";
	fout << std::to_string(this->sn_);
	fout.close();
...
void MyTask::load(std::string filename)
{
  ...
  // TODO
	...
	while (!fin.eof()) {
		fin >> cache;
		this->data_.push_back(cache);
		std::cout << this->data_.back();
	}
	fin.close();
...
//#error "Please finish get_all_prime_divisors(n) function"
	std::list<uint64_t> your_result;
	for (uint64_t i = n / 2; i > 1; i--) {
		if (n % i == 0 && is_prime(i)) {
			your_result.push_back(i);
		}
	}
